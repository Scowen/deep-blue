...
public class Buoyancy : MonoBehaviour{
	...
	private void Start(){
		...
		meshCol = target.GetComponent<MeshCollider>();
		...
		if(rigid == null){
			//            Debug.LogWarning(string.Format("[Buoyancy.cs] Object \"{0}\" had no Rigidbody.", name));
			enabled = false;
		}
		rigid.centerOfMass = new Vector3(0, -bounds.extents.y * 0f, 0) + transform.InverseTransformPoint(bounds.center);
		...
		float volume = rigid.mass / density;
		...
		//        Debug.Log(string.Format("[Buoyancy.cs] Name=\"{0}\" volume={1:0.0}, mass={2:0.0}, density={3:0.0}", name, volume, rigid.mass, density));
	}
	...
	private void FixedUpdate(){
		...
		foreach(var point in voxels){
			...
			if(wp.y - voxelHalfHeight < waterLevel){
				...
				var velocity = rigid.GetPointVelocity(wp);
				var localDampingForce = -velocity * DAMPFER * rigid.mass;
				var force = localDampingForce + Mathf.Sqrt(k) * localArchimedesForce;
				rigid.AddForceAtPosition(force, wp);
	...
	private void OnDrawGizmos(){
		...
		foreach(var force in forces){
			...
			Gizmos.DrawLine(force[0], force[0] + force[1] / rigid.mass);
		}
	}
}